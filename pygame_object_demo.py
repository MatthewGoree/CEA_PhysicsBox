...
#ball.radius = int(ball_size[0] / 2)
#ball.volume = 4 / 3 * numpy.pi * ball.radius**3


#buoyancy = ball.volume * gravity * liquid.density
#mass = material.density * ball.volume
...
while 1:
    ...
    print("speed length is ", len(speed))

    ball.rect = ball.rect.move(speed)
    #print("tmove is", tmove)
    ...
    if ball.rect.bottom > liquid_height:
        ...
        if ball.rect.top < liquid_height:
            ...
            if ball.radius >= h:
                ...
                vol_submerged = numpy.pi * h ** 2 / 3 * (3 * ball.radius - h)
                # Central angle of partial circle
                theta = 2 * numpy.arccos((ball.radius - h) / ball.radius)
            else:
                h_temp = 2 * ball.radius - h
                r_submerged = 2 * numpy.tan(numpy.arccos((h - ball.radius) / ball.radius)) * (h - ball.radius)
                vol_submerged = ball.volume - numpy.pi * h_temp ** 2 / 3 * (3 * ball.radius - h_temp)
                # Central angle of partial circle
                theta = 2 * numpy.arccos(h_temp / ball.radius)
            #area_submerged = numpy.pi * r_submerged**2
            # Ratio of area submerged to total area
            ratio = (ball.radius**2 / 2 * (theta - numpy.sin(theta))) / (numpy.pi * ball.radius**2)
            buoy_submerged = vol_submerged * liquid.density * gravity
            acc_b = buoy_submerged / ball.mass
            # Modified Stokes' flow
            Fdx = 6 * numpy.pi * liquid.viscosity * ball.radius * speed[0] * ratio
            Fdy = 6 * numpy.pi * liquid.viscosity * r_submerged * speed[1]
        else:
            acc_b = buoyancy(ball, liquid, gravity)
            #acc_b = ball.buoyancy / ball.mass
            # Stokes' flow
            Fdx = 6 * numpy.pi * liquid.viscosity * ball.radius * speed[0]
            Fdy = 6 * numpy.pi * liquid.viscosity * ball.radius * speed[1]
        ...
        print("acc_dy is ", acc_dy)
        print("buoyancy is ", acc_b)
        print("mass is ", ball.mass)
        print("vol is ", ball.volume)
